...
## 2026-02-08 UI修正・接続チェック強化

### 依頼内容
1. Notionへ保存ダイアログの入力欄が幅を超えて表示される問題を修正する。
2. 「Notionへ保存」ボタンを押した際、Notion接続が無効な場合に即座に接続エラーダイアログを表示する（プレビューダイアログを表示する前にチェックを行う）。

### 変更点
- **UI修正**:
     - **`styles.css`**: 入力欄 (`input`, `textarea`) およびダイアログコンテナに `box-sizing: border-box` を適用し、paddingによる幅溢れを解消。
- **機能改善**:
     - **`background.js`**: `checkNotionConnection` 関数を追加し、API KeyとDatabase IDの有効性をNotion APIへの実際の問い合わせで確認するロジックを実装。
     - **`shared/flows/exportFlow.js`**: 「Notionへ保存」フローの冒頭で `checkNotionConnection` を呼び出し、接続エラー時は即座に「接続設定」ダイアログを表示するように変更。これにより、保存しようとした段階で設定不備に気づけるようになった。
- **UI調整**:
     - **ボタン文言変更**: 「まとめを作成」ボタンのテキストを「チャットを要約」に変更し、機能をより直感的に表現。
